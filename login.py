...
class Login_window:
    def __init__(self, root):
        ...
        chngpaswrd_btn= Button(login_frame, cursor= "hand2", text= "Forgot Your Password?", command= self.forgot_password_window, font= ("times new roman", 12, "bold"), bg= "#edf2f4", fg= "#d90429").place(x= 460, y= 300)
        ...
        self.clock_lbl.place(x= 90, y= 25, height= 450, width= 350)
    ...
    def login(self):
        if self.email_ntr.get()== "" or self.paswrd_ntr.get()=="":
            ...
            try:
                cnct= pymysql.connect(user= "lifechoices",
                                        host="localhost",
    ...
    def reset(self):
        ...
        self.newpaswrd_ntr.delete(0, END)
        self.ans_ntr.delete(0, END)
        self.paswrd_ntr.delete(0, END)
        self.email_ntr.delete(0, END)

    def forgot_password(self):
        if self.scritqustn_drpdwn.get()== "Select" or self.ans_ntr.get()== "" or self.newpaswrd_ntr.get()== "":
            messagebox.showerror("Error", "Please Fill In All Fields.", parent= self.root2)
        else:
            try:
                cnct= pymysql.connect(user= "lifechoices",
                                        host="localhost",
                                        ...
                                        password= "@Lifechoices1234")
                crsr= cnct.cursor()
                crsr.execute("select * from lifechoicesonline where email=%s and scrit_qustin=%s aand answer=%s", (self.email_ntr.get(), self.scritqustn_drpdwn.get(), self.ans_ntr.get()))
                row= crsr.fetchone()
                if row== None:
                    messagebox.showerror("Error!", "Please Select The Correct SECURITY QUESTION & Supply Correct ANSWER.", parent= self.root2)
                else:
                    crsr.execute("update lifechoicesonline set confrm_paswrd=%s where email=%s", (self.confrmpaswrd_ntr.get(), self.email_ntr.get()))
                    cnct.commit()
                    cnct.close()
                    messagebox.showinfo("Good!", "You Have Set a NEW PASSWORD, You Can Now LOGIN With The Reset PASSWORD.", parent= self.root2)
                    self.reset()
                    self.root2.destroy()
        ...
        if self.email_ntr.get()== "":
            ...
            try:
                ...
                if row== None:
                    ...
                    self.scritqustn_drpdwn['values']= ("Select---", "Your Mother's Maiden Name", "Your Home Town", "Your Best Friend")
                    ...
                    chngpaswrd_btn= Button(self.root2, command= self.forgot_password, text= "Reset Your Password", bg= "#8d99ae", fg= "#d90429", font= ("times new roman", 18, "bold"))     
